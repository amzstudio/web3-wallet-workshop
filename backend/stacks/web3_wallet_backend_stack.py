...
class Web3WalletBackendStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # GET /wallet/step0/{address}?nextToken={nextToken}&maxResults={maxResults}
        step0_get_toke_balances = wallet.add_resource("step0").add_resource("{address}").add_method("GET",method_responses=[api_gw.MethodResponse(status_code='200',response_models={'application/json': api_gw.Model.EMPTY_MODEL})],request_parameters={"method.request.querystring.nextToken": False,"method.request.querystring.maxResults": False})

        # GET /wallet/step1/{address}?nextToken={nextToken}&maxResults={maxResults}
        step1_list_token_balances = wallet.add_resource("step1").add_resource("{address}").add_method("GET",method_responses=[api_gw.MethodResponse(status_code='200',response_models={'application/json': api_gw.Model.EMPTY_MODEL})],request_parameters={"method.request.querystring.nextToken": False,"method.request.querystring.maxResults": False})
