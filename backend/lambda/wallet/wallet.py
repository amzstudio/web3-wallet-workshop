...
def handler(event, context):
    ...
    # start # 
    if functions == "step0":
        res = step0_get_token_balance(path_params, query_params)
    
    if functions == "step1":
        res = step1_list_token_balances(path_params, query_params)
        
    # wallet see the transaction List
    elif functions == "step2":
        res = step2_list_transactions(path_params, query_params)

    # wallet see the transaction in detail
    elif functions == "step3":
...
# step 0
def step0_get_token_balance(path_params, query_params):
...
# step 1
def step1_list_token_balances(path_params, query_params):
...
def step3_get_transaction_detail(path_params, query_params):
    ...
    try: 
        ...
        if transaction["ResponseMetadata"]["HTTPStatusCode"] == 200:
            ...
            transaction_event = query.list_transaction_events(network = path_params["network"], transactionHash=trxId)
